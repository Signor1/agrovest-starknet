...
import MenuButton from "@/app/components/internal/MenuButton";
      ...
      >
        <MaxWrapper className="h-full w-full flex items-center justify-between">
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            {/* Mobile */}
            <div
              className={`fixed top-0 z-[99] w-full h-[100dvh] bg-lightgreen transition-all duration-[500ms] ease-[cubic-bezier(0.86,0,0.07,1)] lg:hidden flex justify-end ${showMobileNav ? "left-0" : "left-[100%]"
                }`}
            >
              <div
                className={`w-[80%] h-full bg-darkgreen flex flex-col gap-10 transition-all duration-[500ms] ease-[cubic-bezier(0.86,0,0.07,1)] px-8 py-8 delay-300 ${showMobileNav ? "translate-x-0" : "translate-x-full"
                  }`}
              >
                    ...
                    <li className="block relative list-none group" key={index}>
                      <Link
                        className={`text-sm font-bold uppercase text-gray-100 block leading-none relative tracking-[0.8px] z-[1] font-barlow before:content-[''] before:absolute before:w-[7px] before:h-[7px]
                                            before:rounded-full before:opacity-0 before:transition-all before:duration-[0.3s] before:ease-[ease-out] before:delay-[0s]  before:top-1 before:-left-3 before:bg-lightgreen group-hover:text-lightgreen  group-hover:before:opacity-100 ${pathname === link.href &&
